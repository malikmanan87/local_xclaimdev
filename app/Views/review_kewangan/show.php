...
<div class="row g-4 mb-4">
    ...
    <!-- Left Column: Details, Procedures & JPPP Endorsement -->
    <div class="col-lg-8">
    ...
    <!-- Right Column: Finance Action Panel -->
    <div class="col-lg-4">

        <?php if ($isPendingFin): ?>
            <!-- Form Tindakan Kewangan -->
            <div class="card border-0 shadow-sm rounded-3 border-top border-success border-4">
                <div class="card-header bg-white py-3">
                    <h6 class="fw-bold text-dark mb-0">
                        <i class="bi bi-cash-stack text-success me-2"></i>Kelulusan Baucar Bayaran
                    </h6>
                    <small class="text-muted">Sila buat pengesahan bayaran dan jana nombor baucar.</small>
                </div>
                <div class="card-body p-3">
                    <form id="formFinanceAction">
                            ...
                            <div class="d-flex flex-column gap-2">
                                <label class="form-check-label border rounded p-2.5 d-flex align-items-center gap-2 cursor-pointer bg-light hover-shadow" for="actionApprove">
                                    ...
                                    <div>
                                        ...
                                        <small class="text-muted">Sedia untuk pembayaran kepada pakar.</small>
                                    </div>
                                </label>
                        ...
                        <div class="mb-3" id="voucherFieldGroup">
                            ...
                            <input type="text" class="form-control form-control-sm font-monospace fw-semibold text-primary" id="voucherNo" name="voucher_no" value="<?= esc($defaultVoucherNo) ?>" placeholder="Contoh: VCR-20260920-0001">
                        ...
                        <div class="mb-3">
                            <label for="financeRemarks" class="form-label fw-semibold small text-secondary">
                                Catatan / Rujukan Kewangan:
                            </label>
                            <textarea class="form-control form-control-sm" id="financeRemarks" name="remarks" rows="2" placeholder="Masukkan sebarang catatan transaksi, akaun atau cek jika ada..."></textarea>
                        </div>

                        <div class="p-3 bg-light rounded-3 mb-3 border text-center">
                            <div class="small text-muted">Jumlah Bayaran Yang Akan Diluluskan:</div>
                        ...
                        <div class="d-grid pt-2">
                            <button type="button" class="btn btn-success fw-semibold shadow-sm" id="btnSubmitFinance">
                                <i class="bi bi-check2-circle me-1"></i> Sahkan & Luluskan Pembayaran
                            </button>
                        </div>
                    </form>
                </div>
        ...
        <?php else: ?>
            ...
            <div class="card border-0 shadow-sm rounded-3 <?= $isApprovedFin ? 'border-top border-success border-4' : 'border-top border-danger border-4' ?>">
                <div class="card-header bg-white py-3">
                    <h6 class="fw-bold text-dark mb-0">
                ...
                <div class="card-body p-3">
                    <div class="text-center py-2 mb-3">
                        <span class="badge <?= $isApprovedFin ? 'bg-success' : 'bg-danger' ?> fs-6 px-3 py-2 rounded-pill">
                    ...
                    <table class="table table-sm table-borderless small mb-0">
                        <?php if ($isApprovedFin && !empty($application['finance_voucher_no'])): ?>
                            <tr>
                                <th width="40%" class="text-muted">No. Baucar:</th>
                                <td class="font-monospace fw-bold text-primary"><?= esc($application['finance_voucher_no']) ?></td>
                            </tr>
                        <?php endif; ?>
                        <tr>
                            <th width="40%" class="text-muted">Pegawai Kewangan:</th>
                            <td class="fw-bold"><?= esc($application['finance_reviewer_name'] ?? 'Pegawai Kewangan') ?></td>
                        </tr>
                        <tr>
                            <th class="text-muted">Tarikh Kelulusan:</th>
                            <td><?= $application['finance_verified_at'] ? date('d/m/Y h:i A', strtotime($application['finance_verified_at'])) : '-' ?></td>
                        </tr>
                        <tr>
                            <th class="text-muted">Jumlah Bayaran:</th>
                            <td class="font-monospace fw-bold text-success">RM <?= number_format($application['total_claim'] ?? 0, 2) ?></td>
                        </tr>
                        <tr>
                            <th class="text-muted">Catatan:</th>
                            <td class="text-wrap"><?= esc($application['finance_remarks'] ?: 'Tiada catatan.') ?></td>
                        </tr>
                    </table>
                </div>
            </div>
        ...
        <div class="card border-0 shadow-sm rounded-3 mt-4 bg-light">
            <div class="card-body p-3 small">
                <div class="fw-bold text-dark mb-2"><i class="bi bi-clock-history me-1 text-primary"></i> Jejak Audit Permohonan:</div>
                <ul class="list-unstyled mb-0 position-relative ps-2">
                    <li class="mb-2.5">
                        <i class="bi bi-check-circle-fill text-success me-2"></i>
                        <strong>1. Dihantar oleh Pakar</strong>
                        <div class="text-muted ps-4"><?= date('d/m/Y h:i A', strtotime($application['created_at'])) ?></div>
                    </li>
                    <li class="mb-2.5">
                        <i class="bi bi-check-circle-fill text-success me-2"></i>
                        <strong>2. Pengesahan JPPP</strong>
                        <div class="text-muted ps-4">
                            Disokong oleh <?= esc($application['jppp_reviewer_name'] ?? 'JPPP') ?><br>
                            <?= $application['jppp_verified_at'] ? date('d/m/Y h:i A', strtotime($application['jppp_verified_at'])) : '-' ?>
                        </div>
                    </li>
                    <li>
                        <i class="bi <?= $isApprovedFin ? 'bi-check-circle-fill text-success' : ($isRejectedFin ? 'bi-x-circle-fill text-danger' : 'bi-circle text-warning') ?> me-2"></i>
                        <strong>3. Kelulusan Kewangan</strong>
                        <div class="text-muted ps-4">
                            <?= $isPendingFin ? 'Sedang Diproses' : ($isApprovedFin ? 'Diluluskan untuk Bayaran' : 'Ditolak') ?>
                        </div>
                    </li>
                </ul>
            </div>
